...
<body>
	<form action="<%=request.getContextPath()%>/ServletPostPelicula" method="post" target="">
		...
		<tbody>
			<c:forEach items="${lstPeliculas}" var="peli">
				<tr>
					...
					<td><a href="<%=request.getContextPath()%>/ServletPostPelicula?accion=agregar&codPeli=${peli.getCodigo()}&nombre=${peli.getNombre()}&duracion=${peli.getDuracion()}">Agregar</a>
